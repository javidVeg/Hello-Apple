...
export const getPosts = async (req, res) => {
    ...
    try {
        ...
        const total = await PostProduct.countDocuments({});
        const posts = await PostProduct.find().sort({ _id: -1 }).limit(LIMIT).skip(startIndex);
...
export const getPostsBySearch = async (req, res) => {
    ...
    try {
        ...
        const posts = await PostProduct.find({ $or: [ { product }, { tags: { $in: tags.split(',') } } ]});
...
export const getPostsByCreator = async (req, res) => {
    ...
    try {
        const posts = await PostProduct.find({ name });
...
export const createPost = async (req, res) => {
    ...
    const newPostProduct = new PostProduct({ ...post, creator: req.userId, createdAt: new Date().toISOString() })
